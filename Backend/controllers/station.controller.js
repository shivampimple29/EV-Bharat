...
module.exports.getNearbyStations = async (req, res) => {
  ...
  // ✅ count only nearby, not all stations
  const total = await EVStation.countDocuments({
    status: "approved",
    location: {
      $near: {
        $geometry: {
...
module.exports.getAllStationsForMap = async (req, res) => {
  const stations = await EVStation.find({ status: "approved" })
...
module.exports.getAllStations = async (req, res) => {
  ...
  if (chargerType.trim()) {
    filter["chargers.type"] = { $regex: chargerType, $options: "i" };
  }
